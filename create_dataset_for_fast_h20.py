...
def get_training_image_pair(files_train,imageId):
    ...
    path = os.path.join(os.getcwd(),"Data_masks/Fast_H20/" + imageId + "_Fast_H20.tif")
...
def create_training_batch(file_train,cover = 0.1,batch_size = 16,img_rows = 112,img_cols = 112,num_channels = 14):
   ...
   # Only one training image for water. Hence "0"
   # is the only index.
   ...
   # Find indices where water is present.
   x_1s,y_1s,z_1s = np.where(truth1 == 255)

   pixel_cords = [(x_1s,y_1s,z_1s)]

   # Total batch size is 100.
   percentage = 0
...
def save_data(X,Y,i):
    ...
    '''
    with open("./Data_training/Fast_H20/train" + "_" + str(i) + "_" + ".pkl","wb") as f:
        pickle.dump(X,f)
    
    with open("./Data_training/Fast_H20/test" + "_" + str(i) + "_" + ".pkl","wb") as f:
        pickle.dump(Y,f)
...
if __name__ == '__main__' :
    ...
    for i in range(0,100) :
        X,Y = create_training_batch(file_train,cover,batch_size,img_rows,img_cols,num_channels)
        save_data(X,Y,i)
        print(i)
